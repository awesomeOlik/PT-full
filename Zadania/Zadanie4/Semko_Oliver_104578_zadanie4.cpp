...
size_t partition(int *data, const size_t pivot, const size_t low, const size_t high)
{
    ...
    while(compare_index < high)
    {
        if(data[compare_index] >= data[high-1]){
            swap(&data[compare_index], &data[insert_index]);
            insert_index++;
        }
        compare_index++;
    }
...
int main() {
    ...
    //1. int data[] = {7, 1, 34, 16, 12, 44, 19, 99, 0, 4};int len = sizeof(data)/sizeof(data[0]);bubbleSort(data, len);printArray(data, len);
    //int data[]= {10, 20, 5000, 30, 1000, 40, 2000, 50, 60, 70}; /*size_t pivot= 4;*/size_t low= 2;size_t high= 7;
    //int len = sizeof(data)/sizeof(data[0]);
    //cout << partition(data,pivot, low, high) << endl;
    //cout << getPivotIndex(data, low, high) << endl;
    //printArray(data, len);

    /*Weight data[] = {{10, 1}, {20, 2}, {5,2}};
    int len = sizeof(data)/sizeof(data[0]);
    bubbleSort(data, len);*/


    /*int data[] = {7, 1, 34, 16, 12, 44, 19, 99, 0, 4};
    int len = sizeof(data)/sizeof(data[0]);
    printArray(data, len);
    cout << endl;
